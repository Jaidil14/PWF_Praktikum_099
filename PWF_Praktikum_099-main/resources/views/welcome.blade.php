            ...
            @if (Route::has('login'))
                @auth
                    <a href="{{ url('/dashboard') }}"
                        class="col-span-2 bg-white text-black px-4 py-2 rounded-md font-medium hover:bg-gray-200 transition-colors text-center">
                        Masuk ke Dashboard
                    </a>
                @else
                    <a href="{{ route('login') }}"
                        class="bg-white text-black px-4 py-2 rounded-md font-medium hover:bg-gray-200 transition-colors text-center">
                        Login
                    </a>

                    @if (Route::has('register'))
                        <a href="{{ route('register') }}"
                            class="border border-[#3E3E3A] text-white px-4 py-2 rounded-md font-medium hover:bg-[#262624] transition-colors text-center">
                            Register
                        </a>
                    @endif
                @endauth
